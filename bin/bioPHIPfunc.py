    ...
    def main_loop(self):
        ...
        if self.par['phip_merge'] == 'yes':
            ...
            if 'file_annotation' in self.par:
                ...
                arg_tuple = ('_pro_maxRC.txt', RC_level, out_file, self.par['pro_ids'])
                args_list.append(arg_tuple)
                #prosum level
                out_file = self.par['files_dict']['prosum_RC']
                arg_tuple = ('_pro_sumRC.txt', RC_level, out_file, self.par['pro_ids'])
                args_list.append(arg_tuple)
            #multi-threads
        ...
        if self.par['phip_GP'] == 'yes':
            ...
            if 'VirScan' in self.par['file_annotation']:
                ...
                pool.apply_async(self.taxon_spec, args=(pep_zfile, 'taxon_phip', 'pep_id',))
        ...
        if self.par['phip_enrichment'] == 'yes':
            ...
            for term in terms:
                pool.apply_async(E.enrich_pro, args=(prosum_zfile, 'pro_id', term, ',', None,))
                pool.apply_async(E.enrich_pro, args=(promax_zfile, 'pro_id', term, ',', None,))
        pool.close()
        pool.join()
################################
...
#get RC matrix by proteins
    def combine_peptides(self,sample_var):
        ...
        sub_annot_df = annot_df[['pep_id', 'pro_id']]
        #print(sub_annot_df)
        #read RC file
        ...
        select_col.insert(0, 'pro_id')
        comb_df = pd.merge(sub_annot_df, RC_df, left_on='pep_id', right_on='row_name')
        comb_df = comb_df[select_col]
        #print(comb_df)
        #group by 'pro_id'
        pro_sumRC_df = comb_df.groupby('pro_id', axis=0).sum()
        #print(pro_sumRC_df)
        #export to file
        pro_sumRC_df.to_csv(sample_var['sample_pro_sumRC_file'], sep="\t")
        #group by sample_par['pro_id']
        pro_maxRC_df = comb_df.groupby('pro_id', axis=0).max()
        #export to file
        pro_maxRC_df.to_csv(sample_var['sample_pro_maxRC_file'], sep="\t", index_label='pro_id')
...
#virus taxonomy specifications
    def taxon_spec(self, count_file,taxon_rank,annot_index):
        ...
        #myDict.basic(taxon_pval_dict).dict2_to_file(file_head+'pvalues.txt', "\t")
...
#polyclonal
    def sig_polyclonal(self, count_file):
        ...
        myDict.basic(hits1, self.par['pro_ids']).dict2_to_file(file_head+'.txt', "\t")
        myDict.basic(hits2, self.par['pro_ids']).dict2_to_file(file_head+'_nonoverlapped.txt', "\t")
        ...
        for sample_name, column in zdf.items():
            ...
            if input_num > 0:
                ...
                try:
                    ...
                    myIO.file_os(self.par['file_err'], "\t").line_replace({'taxon_blast':sample_name})
