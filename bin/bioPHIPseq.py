...
#initiate default dir and file
    def init_dir_file(self):
        ...
        if 'VirScan' in self.par['genome_index_name']:
            self.par['lib'] = 'virus'
            self.par['file_NC'] = self.par['dir_ref_seq'] + 'virus_pep_RC_BeadsOnly.txt'
        elif 'human' in self.par['genome_index_name']:
            self.par['lib'] = 'human'
            self.par['file_NC'] = self.par['dir_ref_seq'] + 'human_pep_RC_BeadsOnly.txt'
        elif 'Allergome' in self.par['genome_index_name']:
            self.par['lib'] = 'allergome'
            self.par['file_NC'] = self.par['dir_ref_seq'] + 'allergome_pep_RC_BeadsOnly.txt'
        elif 'ProVirome' in self.par['genome_index_name']:
            self.par['lib'] = 'provirome'
            self.par['file_NC'] = self.par['dir_ref_seq'] + 'provirome_pep_RC_BeadsOnly.txt'
        elif 'Toxome' in self.par['genome_index_name']:
            self.par['lib'] = 'toxome'
            self.par['file_NC'] = self.par['dir_ref_seq'] + 'toxome_pep_RC_BeadsOnly.txt'
        elif 'mouse' in self.par['genome_index_name']:
            self.par['lib'] = 'mouse'
        elif 'PublicEpitope' in self.par['genome_index_name']:
            self.par['lib'] = 'PE'
        elif 'Zika' in self.par['genome_index_name']:
            self.par['lib'] = 'zika'        
        elif 'ArboScan' in self.par['genome_index_name']:
            self.par['lib'] = 'arbo'        
        ...
        self.par['file_err'] = self.par['dir_result'] + 'error.log'
...
#initiate analysis 
    def init_analysis(self):
        ...
        #1. read reference sequence file (*.fa)
        self.par['ref_dict'], self.par['pep_ids'] = myGenome.genome(self.par['file_ref_fa']).read_fa()
        #read annotation file
        if 'file_annotation' in self.par.keys():
            self.par['annot_df'] = myDataframe.basic().annot_df(self.par['file_annotation'])
            #export pro_ids removing duplicates and preserve the original order
            pro_all=list(self.par['annot_df']['pro_id'])
            self.par['pro_ids']=sorted(set(pro_all), key=pro_all.index)
            print(self.par['pro_ids'])
