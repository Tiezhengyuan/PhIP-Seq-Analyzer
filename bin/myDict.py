...
class basic:
    ...
    def __init__(self, dictionary=None, dict_keys=None):
        ...
        if isinstance(self.dict, dict):
            self.dict_keys=myList.basic(self.dict.keys()).sort_list() if dict_keys is None else dict_keys
        self.out_dict={}
...
#initiate nested dictionary as all zeros all something else    
    ...
    def dict_to_file(self, out_file, pattern='='):
        ...
        for key in self.dict_keys:
            value= str(self.dict[key]) if key in self.dict.keys() else 'NA'
            line = ''.join([str(key),str(pattern),value,'\n'])
            out_obj.write(line)
        out_obj.close()
    ...
    def dict2_to_file(self, out_file, pattern='\t', NA='0', index_label='row_name'):
        keys2 = self.nested_keys()
        #print keys2
        #
        out_obj = open(out_file, 'wt')
        ...
        for key1 in self.dict_keys:
            ...
            if key1 in self.dict.keys():
                sub_dict = self.dict[key1]
                for key2 in keys2:
                    value = sub_dict[key2] if key2 in sub_dict else NA
                    items.append(value)
            else:
                items += [NA]*len(keys2)
            #write a line to out_file
