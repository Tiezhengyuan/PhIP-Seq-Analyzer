...
#based on barcode of one index file
    def decompose_fq(self, par):
        ...
        print(dash_line)
        print(stdout_format.format('Raw reads','Assigned reads', 'Percentage', 'Trim reads'))
        ...
        with F1, F2:
            ...
            for L1,La, L2,Lb, L3,Lc, L4,Ld in itertools.zip_longest(*[F1,F2]*4):
                ...
                if n >= 1e5 and n%5e5 == 0: #million
                    ...
                    flen=len(L2)-1
                    read_info="{}-->{}".format(rlen,flen) if tag is True else "{}-->X".format(rlen)
                    print(stdout_format.format(n/1e6, m/1e6, perc, read_info))
                #if n==3e6: break
            else:
                print(dash_line)
                print(stdout_format.format(n/1e6, m/1e6, round(m*100/n, 2), '---'))
                print(dash_line)
...
#based on barcode of two index file
    def decompose_fq2(self, par):
        ...
        with F1, F2, F3:
            #read 4 lines at a time per file
            for L1,La,Le, L2,Lb,Lf, L3,Lc,Lg, L4,Ld,Lh in itertools.zip_longest(*[F1,F2,F3]*4):
...
#shrink fastq file and index file matched based on barcode file
