...
#calculate z scores against negative controls based on robust linear regression
    def NCPHIPzscores_RLM(self):
        ...
        try:
            ...
            NC_df['pred_std']=1
            err_dict={'myStat.NCPHIPzscores_RLM()':'NC_df regression'}
            myIO.file_os(self.par['file_err'], "\t").line_replace(err_dict)
        finally:
        ...
        for sample in self.par['sample_names']:
            ...
            try:
                ...
                myIO.file_os(self.par['file_err'], "\t").line_replace({'myStat.NCPHIPzscores_RLM()':sample})
            #calculate z-score
...
#self.par['file_NC'], self.par['scaling_factor']
    def NC_whole_std(self):
        ...
        file_prefix = '{}{}_'.format(self.par['dir_stat'], myIO.file_os(self.par['file_NC']).name_prefix())
        ...
        #match rows between phip_nc and self.indf
        phip_nc=phip_nc.ix[list(self.indf.index)].fillna(0).copy()
        ...
        plot_par={'df': NC[['logmedian','logstd']], 'xlim':(-.5,xm), 'ylim':(-.5,ym),\
                  'picfile':file_prefix+'polynomial_std.png', 'text':str(pn_fit.params) }
        try:
            ...
            err_dict={'myStat.NC_whole_std()':'plot of STD~median'}
            myIO.file_os(self.par['file_err'], "\t").line_replace(err_dict)
